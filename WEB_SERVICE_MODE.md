...
This will:
1. Start a callback server on port 8001
2. Send a test annotation request
3. Display results when submitted/cancelled

## Notes

- Only one annotation request can be processed at a time
- Service automatically rejects new requests while busy
- Layer configuration is temporarily updated for each request
- Original layers.txt is restored after each session
