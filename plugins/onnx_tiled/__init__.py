...
def preprocess_patch(patch_np, layout, channels):
    ...
    if channels == 1:
        ...
        # OJO: cv2.imread(..., IMREAD_COLOR) devuelve BGR.
        # No convertir a RGB.
        if layout == "NCHW":
            patch_np = np.transpose(patch_np, (2, 0, 1))[None, :, :, :]  # 1,3,H,W
        else:
...
def standardize_output(pred):
    ...
    # CHW -> HWC si parece channels_first
    if pred.ndim == 3 and pred.shape[0] in (1, 2, 3) and pred.shape[-1] not in (1, 2, 3):
        pred = np.transpose(pred, (1, 2, 0))
...
class CapitanPlugin(AutolabelPlugin):
    """ONNX-backed autolabeling plugin for the Capitan model family."""

    def __init__(self):
        self._logger = logging.getLogger("CapitanPlugin")
        plugin_dir = os.path.dirname(os.path.abspath(__file__))

        # Discover layers from sorted .onnx filenames
        onnx_files = sorted(
            f for f in os.listdir(plugin_dir) if f.endswith(".onnx")
        )
        if not onnx_files:
            raise RuntimeError(
                f"No .onnx files found in capitan plugin directory: {plugin_dir}"
            )

        self._layer_names: list = [os.path.splitext(f)[0] for f in onnx_files]
        model_paths = [os.path.join(plugin_dir, f) for f in onnx_files]

        # Load all ONNX sessions eagerly so load errors surface at discovery time
        self._sessions: list = []
        for path in model_paths:
            ...
            self._logger.info(
                "Loaded ONNX model: %s", os.path.basename(path)
            )
            self._sessions.append(session)
    ...
    def run(self, image: np.ndarray) -> np.ndarray:
        """Run all ONNX models and return a full-image label map.

        Parameters
        ----------
        image : np.ndarray
            Input image in BGR format (OpenCV), shape (H, W, C).

        Returns
        -------
        np.ndarray
            2-D label map (H, W, dtype=uint8).  Each pixel value is an
            index into ``supported_layers`` — the layer whose model
            produced the highest probability for that pixel.
        """
        prob_maps = []
        for session, layer_name in zip(self._sessions, self._layer_names):
            prob = self._run_model(session, image, layer_name)
        ...
        # Stack → (H, W, N_layers); argmax selects the winning layer per pixel
        prob_stack = np.stack(prob_maps, axis=-1)
        label_map = np.argmax(prob_stack, axis=-1).astype(np.uint8)
        return label_map
    ...
    def _run_model(
        self, session: ort.InferenceSession, image: np.ndarray, layer_name: str
    ) -> np.ndarray:
        """Run one ONNX model on the full image using tiled inference.

        Returns a float32 probability map of shape (H, W) with values in
        [0, 1].
        """
        input_info = session.get_inputs()[0]
        input_name = input_info.name
        ...
        # Convert the image to the channel count expected by this model
        img_np = self._prepare_channels(image, channels)
    ...
    @staticmethod
    def _prepare_channels(image: np.ndarray, channels: int) -> np.ndarray:
        """Return a copy of *image* with the correct number of colour channels."""
        if channels == 1:
            if image.ndim == 3:
                return cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
            return image.copy()
