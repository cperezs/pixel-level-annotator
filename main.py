...
class PixelAnnotationApp(QMainWindow):
    ...
    def cb_toggle_web_service_mode(self):
        ...
        if is_enabled:
            ...
            # Ocultar lista y etiqueta de imágenes
            self.q_image_list.setVisible(False)
            self.q_image_label.setVisible(False)
            self._logger.info("Web service mode enabled")
        else:
            # Stop web service
            self.web_service.stop_server()
            # Mostrar lista y etiqueta de imágenes
            self.q_image_list.setVisible(True)
            self.q_image_label.setVisible(True)
    ...
    def create_layer_buttons(self, layers):
        """Crea los botones de capa a partir de una lista de diccionarios {'name', 'color'}."""
        self.q_layer_buttons = []
        self.hex_colors = [layer["color"] for layer in layers]
        self.colors = [tuple(int(color[i:i+2], 16) for i in (1, 3, 5)) for color in self.hex_colors]
        for i, layer in enumerate(layers):
            label = f" ({str(i + 1)})" if i < 9 else ""
            button = QPushButton(f"{layer['name']}{label}")
    ...
    def load_web_service_image(self, image_path, request):
        ...
        try:
            ...
            # Keep current layer configuration; do not reload on web service requests
    ...
    def reload_layers_config(self, layers=None):
        """Recarga la configuración de capas y actualiza la UI. Si layers es None, lee de layers.txt"""
        try:
            ...
            if layers is None:
                layers = self.read_layers_file()

            self.create_layer_buttons(layers)
    ...
    def cb_submit_annotations(self):
        ...
        try:
            ...
            annotation_images = {}
            # Derive layer names from UI buttons (text before optional numbering label)
            layer_names = [btn.text().split(' (')[0] for btn in self.q_layer_buttons]
